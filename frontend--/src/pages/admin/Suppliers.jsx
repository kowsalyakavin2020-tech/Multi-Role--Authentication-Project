...
import { Users, Search, CheckCircle, Ban, Eye } from "lucide-react";
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-sm font-medium">Loading suppliers...</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-600 text-sm mt-1 font-medium">
            View, approve and manage all supplier accounts
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        <div className="bg-blue-600 text-white rounded-xl p-4 flex items-center gap-3 shadow">
          ...
          <div>
            <p className="text-blue-100 text-xs font-medium">Total Suppliers</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-4 shadow-sm flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-400"
          />
        </div>
        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value)}
          className="border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
        ...
        <div className="self-center text-sm text-slate-600 font-semibold">
          Showing <span className="text-slate-900">{filtered.length}</span> suppliers
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map((supplier) => (
              <tr key={supplier.user_id} className="hover:bg-slate-50 transition">
                ...
                <td className="px-4 py-3 text-slate-700 font-medium">{supplier.business_name}</td>
                <td className="px-4 py-3 text-slate-700 font-medium">{supplier.mobile}</td>
                <td className="px-4 py-3 text-slate-700 font-medium">{supplier.location}</td>
                <td className="px-4 py-3">
